"use client";

import { motion, MotionValue, useTransform, useMotionValueEvent } from "framer-motion";
import { useState } from "react";

interface Props {
  progress: MotionValue<number>;
}

export default function IconCountingScene({ progress }: Props) {
  const [count, setCount] = useState(1);
  const scale = useTransform(progress, [0, 0.15, 0.85, 1], [0.8, 1, 1, 0.9]);
  const opacity = useTransform(progress, [0, 0.1, 0.9, 1], [0, 1, 1, 0]);

  useMotionValueEvent(progress, "change", (latest) => {
    setCount(Math.max(1, Math.round(1 + latest * 4))); // 1 a 5
  });

  return (
    <motion.div style={{ opacity, scale }} className="relative flex items-center justify-center">
      <div className="w-24 h-24 rounded-full bg-emerald-500 flex items-center justify-center shadow-xl">
        <svg viewBox="0 0 32 32" className="w-12 h-12 fill-white">
          <path d="M16 .5C7.4.5.5 7.4.5 16c0 2.8.7 5.5 2.1 7.9L.5 31.5l7.8-2c2.3 1.3 4.9 2 7.6 2 8.6 0 15.5-6.9 15.5-15.5S24.6.5 16 .5zm0 28.2c-2.4 0-4.7-.6-6.7-1.8l-.5-.3-4.6 1.2 1.2-4.5-.3-.5c-1.3-2.1-2-4.5-2-7 0-7.2 5.9-13.1 13.1-13.1S29 8.8 29 16 22.2 28.7 16 28.7z" />
          <path d="M23.4 19.3c-.4-.2-2.3-1.1-2.6-1.3-.4-.1-.6-.2-.9.2-.3.4-1 1.3-1.2 1.5-.2.2-.4.3-.8.1-.4-.2-1.6-.6-3-1.9-1.1-1-1.9-2.2-2.1-2.6-.2-.4 0-.6.2-.8.2-.2.4-.4.5-.6.2-.2.2-.4.3-.6.1-.2.1-.5 0-.7-.1-.2-.9-2.1-1.2-2.9-.3-.7-.6-.6-.9-.6h-.7c-.2 0-.6.1-.9.5-.3.4-1.2 1.2-1.2 2.8s1.2 3.2 1.4 3.5c.2.2 2.4 3.7 5.9 5.1.8.3 1.5.5 2 .7.8.2 1.6.2 2.2.1.7-.1 2.1-.9 2.4-1.7.3-.8.3-1.5.2-1.7-.1-.1-.4-.2-.8-.4z" />
        </svg>
      </div>
      <div className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-red-500 text-white text-sm font-bold flex items-center justify-center">
        {count}
      </div>
    </motion.div>
  );
}